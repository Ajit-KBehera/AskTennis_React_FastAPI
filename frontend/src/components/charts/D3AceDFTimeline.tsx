...
interface D3AceDFTimelineProps {
  data: RawServeMatch[];
...
/**
 * D3-based Ace/DF Timeline Chart
 * 
 * Key Features:
 * - Point radius = Opponent rank (bigger = tougher opponent)
 * - Shows Ace Rate and Double Fault Rate
 * - Interactive tooltips
 * - Responsive design
 */
export const D3AceDFTimeline: React.FC<D3AceDFTimelineProps> = ({
  data,
  showOpponentComparison = false,
  height = 500,
}) => {
  const metricsToDisplay = useMemo(() => ['player_ace_rate', 'player_df_rate'], []);
  ...
  // Color scheme for Ace/DF metrics
  const getColor = useCallback((metricKey: string, isOpponent: boolean = false): string => {
    if (isOpponent) {
      const colors: Record<string, string> = {
        'player_ace_rate': '#86EFAC', // Light green
        'player_df_rate': '#FCA5A5', // Light red
      };
      return colors[metricKey] || '#94A3B8';
    }
    const colors: Record<string, string> = {
      'player_ace_rate': '#10B981', // Green-500
      'player_df_rate': '#EF4444', // Red-500
    };
    return colors[metricKey] || '#000000';
  }, []);

  // Get metric label
  const getMetricLabel = useCallback((metricKey: string): string => {
    const labels: Record<string, string> = {
      'player_ace_rate': 'Ace Rate',
      'player_df_rate': 'DF Rate',
      'opponent_ace_rate': 'Opponent Ace Rate',
      'opponent_df_rate': 'Opponent DF Rate',
    };
  ...
  // Calculate dynamic Y-axis max (similar to backend logic: cap at 30%, add 15% padding)
  const calculateYMax = useCallback((points: ChartPoint[]): number => {
    if (points.length === 0) return 30;
    const maxValue = Math.max(...points.map(p => p.y));
    return Math.min(30, maxValue * 1.15);
  }, []);
  ...
  useEffect(() => {
    ...
    const yMax = calculateYMax(points);
    ...
    // Draw vertical lines
    const verticalLinesGroup = g.append('g')
      .attr('class', 'vertical-lines');
    
    const verticalLines = verticalLinesGroup
      .selectAll('line')
  ...
  }, [data, metricsToDisplay, showOpponentComparison, dimensions, transformData, calculateYMax, height]);

  return (
    <div className="relative w-full">
      {/* Legend - Metrics (Bottom) */}
      <div className="absolute top-0 left-1/2 transform -translate-x-1/2 bg-white/90 p-3 rounded-lg text-xs text-gray-600 border border-gray-200 z-10 shadow-sm">

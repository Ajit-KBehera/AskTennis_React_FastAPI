...
async def get_serve_stats(request: ServeStatsRequest):
    """
    Get serve statistics charts and raw data for a player.
    """
    if db_service is None:
        raise HTTPException(status_code=500, detail="Database service not initialized")
    ...
    try:
        ...
        # Generate charts (Legacy/Backward Compatibility)
...
async def get_return_stats(request: ReturnStatsRequest):
    """
    Get return statistics charts and raw data for a player.
    """
    if db_service is None:
        raise HTTPException(status_code=500, detail="Database service not initialized")
...
async def get_ranking_stats(request: RankingStatsRequest):
    """
    Get ranking timeline chart and raw data for a player.
    """
    if db_service is None:
        raise HTTPException(status_code=500, detail="Database service not initialized")
    ...
    try:
        ...
        if ranking_fig is None:
            return RankingStatsResponse(
                ranking_data=ranking_data,
                error="Failed to generate ranking chart (insufficient data)" if len(ranking_data) < 2 else None,
                ranking_chart=None
            )

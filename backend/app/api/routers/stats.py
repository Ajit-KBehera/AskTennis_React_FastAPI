...
async def get_serve_stats(request: ServeStatsRequest):
    ...
    try:
        ...
        serve_stats = calculate_aggregated_player_serve_stats(
            matches_with_stats, player_name=player
        )
        ...
        if show_comparison:
            try:
                opponent_stats = calculate_aggregated_opponent_serve_stats(
                    matches_with_stats, opponent_name=opponent
                )
            except Exception:
                pass
        ...
        matches_with_stats["result"] = np.where(
            matches_with_stats["is_winner"], "Win", "Loss"
        )
        ...
        def clean_stats_dict(stats_dict):
            if not stats_dict:
                return None
            return {
                k: (float(v) if not pd.isna(v) else None) for k, v in stats_dict.items()
            }
...
async def get_return_stats(request: ReturnStatsRequest):
    ...
    try:
        ...
        return_stats = calculate_aggregated_player_return_stats(
            matches_with_stats, player_name=player
        )
        ...
        if show_comparison:
            try:
                opponent_stats = calculate_aggregated_opponent_return_stats(
                    matches_with_stats, opponent_name=opponent
                )
            except Exception:
                pass
...
async def get_ranking_stats(request: RankingStatsRequest):
    ...
    try:
        ...
        return RankingStatsResponse(ranking_data=ranking_data)

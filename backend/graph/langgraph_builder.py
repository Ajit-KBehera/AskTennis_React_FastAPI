...
class LangGraphBuilder:
    ...
    def __init__(self, tools: List[Any], llm_with_tools, prompt_template_factory: Callable[[str, str], str], db, full_schema: str):
        """
                ...
                for msg in reversed(messages):
                    if isinstance(msg, HumanMessage):
                        ...
                        if isinstance(content, str):
                            ...
                            user_query = str(content[0].get('text', '')) if isinstance(content[0], dict) else str(content[0])
                        else:
                    ...
                    for tool in self.tools:
                        if tool.name == tool_name:
                            try:
                                ...
                                if tool_name == "sql_db_query":
                                    ...
                                    if isinstance(result, str):
                                        ...
                                        if result_stripped.startswith('[') and result_stripped.endswith(']'):
                                            try:
                                                parsed_result = ast.literal_eval(result_stripped)
                                                if isinstance(parsed_result, list) and len(parsed_result) > 100:
                                                    # Truncate to first 100 rows
                                                    truncated_result = parsed_result[:100]
                                ...
                                return_state["messages"] = [AIMessage(content=str(result), tool_calls=[])]

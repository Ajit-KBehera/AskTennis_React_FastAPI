...
def run_benchmark(limit: int = 5, question_ids: List[int] = None):
    ...
    for i, q in enumerate(questions_to_run):
        print(
            f"Processing ({i + 1}/{len(questions_to_run)}): [ID {q['id']}] {q['question']}"
        )
        ...
        try:
            ...
            if q["id"] in gold_standard:
                ...
                result_entry["gold_expected_keywords"] = gs.get(
                ...
                keywords_met = [
                    k
                    for k in gs.get("expected_answer_keywords", [])
                    if k.lower() in gen_ans_lower
                ]

                is_correct = len(keywords_met) == len(
                    gs.get("expected_answer_keywords", [])
                )
    ...
    if total_checked > 0:
        print(
            f"Accuracy vs Gold Standard: {total_correct}/{total_checked} ({total_correct / total_checked * 100:.1f}%)"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--limit", type=int, default=5, help="Number of questions to run (default: 5)"
    )
    parser.add_argument(
        "--ids",
        type=str,
        help="Comma-separated list of question IDs to run (e.g. 1,5,10)",
    )
    parser.add_argument(
        "--range", type=str, help="Range of question IDs to run (e.g. 1-10)"
    )

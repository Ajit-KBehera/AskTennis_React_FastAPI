...
class DatabaseService:
    ...
    def __init__(self, db_config: Optional[DatabaseConfig] = None, db_engine: Optional[Engine] = None):
        """
        ...
        try:
            with self._get_connection() as conn:
                ...
                if db_type == 'sqlite':
                    ...
                    atp_result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='atp_matches'")).fetchone()
                    wta_result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='wta_matches'")).fetchone()
                    if atp_result and wta_result:
                        return 'separate'
                    # Check for unified matches table (older schema)
                    result = conn.execute(text("SELECT name FROM sqlite_master WHERE type='table' AND name='matches'")).fetchone()
                    if result:
                        return 'unified'
                    ...
                    if db_type == 'postgresql':
                        atp_query = text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'atp_matches')")
                        wta_query = text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'wta_matches')")
                    else:  # mysql
                        atp_query = text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'atp_matches')")
                        wta_query = text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'wta_matches')")
                    ...
                    # Check for unified matches table (older schema)
                    if db_type == 'postgresql':
                        query = text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'matches')")
                    else:  # mysql
                        query = text("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = 'matches')")
        ...
        try:
            ...
            with _self._get_connection() as conn:
                if schema_type == 'unified':
                    ...
                    df = pd.read_sql_query(query, conn, params=tuple(_self._format_params([player_name, player_name])))
                else:
                    # New schema: query both tables
                    query = """
                    SELECT DISTINCT tourney_name
                    ...
                    """
                    query = _self._format_sql_query(query)
                    df = pd.read_sql_query(query, conn, params=tuple(_self._format_params([player_name, player_name, player_name, player_name])))
            if df.empty:
                return [DatabaseService.ALL_TOURNAMENTS]
            return [DatabaseService.ALL_TOURNAMENTS] + df['tourney_name'].tolist()
        except Exception as e:
    ...
    def _get_matches_internal(self, player: Optional[str] = None, 
        ...
        try:
            ...
            if player and player != self.ALL_PLAYERS and opponent and opponent != self.ALL_OPPONENTS:
                ...
                where_conditions.append("((LOWER(winner_name) = LOWER(?) AND LOWER(loser_name) = LOWER(?)) OR (LOWER(winner_name) = LOWER(?) AND LOWER(loser_name) = LOWER(?)))")
                ...
                if player and player != self.ALL_PLAYERS:
                    where_conditions.append("(LOWER(winner_name) = LOWER(?) OR LOWER(loser_name) = LOWER(?))")
    ...
    def get_player_ranking_timeline(_self, player_name: str, year: Optional[Union[int, str, Tuple[int, int], List[int]]] = None) -> pd.DataFrame:
        ...
        try:
            with _self._get_connection() as conn:
                    ...
                    try:
                        ...
                        if isinstance(year, tuple) and len(year) == 2:
                            ...
                            if (_self.MIN_YEAR <= start_year <= _self.MAX_YEAR and 
                                ...
                                year_filter_clause = f"AND CAST({year_expr} AS INTEGER) BETWEEN ? AND ?"
                ...
                atp_df = pd.read_sql_query(atp_query, conn, params=tuple(_self._format_params(all_params)))
                wta_df = pd.read_sql_query(wta_query, conn, params=tuple(_self._format_params(all_params)))

...
class TennisSchemaPruner:
    ...
    def __init__(self, full_schema: str) -> None:
        ...
        self.stat_map = {
            ...
            "break": ["w_bpSaved", "w_bpFaced", "l_bpSaved", "l_bpFaced"],
            "break point": ["w_bpSaved", "w_bpFaced", "l_bpSaved", "l_bpFaced"],
            "breakpoint": ["w_bpSaved", "w_bpFaced", "l_bpSaved", "l_bpFaced"],
        ...
        self._re_create_table = re.compile(
            r"CREATE TABLE\s+(\w+)\s*\([^;]+\);", re.IGNORECASE | re.DOTALL
        )
        self._re_table_content = re.compile(
            r"CREATE TABLE\s+\w+\s*\((.*?)\);", re.IGNORECASE | re.DOTALL
        )
        self._re_table_name = re.compile(r"CREATE TABLE\s+(\w+)", re.IGNORECASE)
    ...
    def prune_schema(self, user_query: str) -> str:
        ...
        for table_name, table_ddl in table_blocks.items():
            if table_name in needed_tables:
                ...
                pruned_ddl = self._prune_table_ddl(table_name, table_ddl, query_lower)
                if pruned_ddl:
    ...
    def _parse_schema(self) -> None:
        ...
        if self._cached_table_blocks is None:
            self._cached_table_blocks = self._split_schema_into_tables(self.full_schema)
    ...
    def _prune_table_ddl(
        ...
        single_word_keywords = {kw for kw in self.stat_keywords if " " not in kw}
        ...
        for line in lines:
            ...
            if col_match:
                ...
                if any(keyword in line_upper for keyword in self.CONSTRAINT_KEYWORDS):
                    filtered_lines.append(line)
        ...
        if not filtered_lines:
            ...
            core_cols_ddl = ",\n    ".join(
                [f"{col} TEXT" for col in self.core_cols.get(table_name, [])]
            )
            table_match = self._re_table_name.match(table_ddl)
            table_name_orig = table_match.group(1) if table_match else table_name
            return f"CREATE TABLE {table_name_orig} (\n    {core_cols_ddl}\n);"
        ...
        table_match = self._re_table_name.match(table_ddl)
        if table_match:
            return f"CREATE TABLE {table_match.group(1)} (\n    {filtered_content}\n);"

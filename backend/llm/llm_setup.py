...
class LLMFactory:
    ...
    @staticmethod
    def create_llm(api_key: str, model: str, temperature: float) -> ChatGoogleGenerativeAI:
        """
    ...
    @staticmethod
    def create_database_connection_from_config(db_config: DatabaseConfig) -> SQLDatabase:
        """
    ...
    def create_sql_tools(db: SQLDatabase) -> List:
        ...
        def sql_db_query_checker(query: str) -> str:
            ...
            if not query_upper.startswith(("SELECT", "WITH")):
                return f"Error: Query must start with SELECT or WITH. Got: {query[:50]}..."
